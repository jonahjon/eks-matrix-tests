...
update-config: prow-config
	kubectl create configmap config --from-file=config.yaml=prow/config/config_all.yaml --dry-run -o yaml | kubectl replace configmap config -f -
